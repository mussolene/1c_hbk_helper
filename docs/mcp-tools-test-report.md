...
**Дата:** 2025-02-26  
**Запрос:** «вывод данных системы компоновки данных в таблицу значений»  
...
|------------|--------|-------------|
| **get_1c_code_answer** | ⚠️ Ошибка → ✅ Исправлено | `bad operand type for unary -: 'NoneType'` при `score=None` в результатах keyword search. Исправлено в `mcp_server.py` (строка 113). |
| **search_1c_help** | ✅ Работает | Находит релевантные темы (Система компоновки данных и т.п.). |
| **search_1c_help_keyword** | ✅ Работает | Точно находит `ПроцессорВыводаРезультатаКомпоновкиДанныхВКоллекциюЗначений` и методы (УстановитьОбъект, НачатьВывод, ЗакончитьВывод, Вывести). |
| **search_1c_help_with_content** | ⚠️ Ошибка → ✅ Исправлено | Та же ошибка score=None; исправление общее с get_1c_code_answer. |
...
**Проверка:** для запроса «вывод СКД в таблицу значений» достаточно 1–2 вызовов (get_1c_code_answer или search_1c_help_keyword + get_1c_help_topic).
